...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function renderNoteList() {
    ...
    filteredNotes.forEach(note => {
      ...
      noteItem.className = `group bg-white rounded-lg shadow-sm border border-gray-100 hover:shadow-md transition-all duration-200 cursor-pointer ${note.id === currentNoteId ? 'ring-2 ring-blue-500' : ''}`;
        ...
        <div class="p-4">
          <div class="flex justify-between items-start">
            <div class="flex-1 min-w-0">
              <div class="text-sm font-medium text-gray-900 truncate mb-2">${note.title || '无标题'}</div>
              <div class="text-xs text-gray-600 mb-2 line-clamp-3 leading-relaxed">${content}</div>
            ...
            <div class="flex space-x-1 ml-3 opacity-0 group-hover:opacity-100 transition-opacity">
              <button class="edit-note-btn p-2 text-gray-500 hover:text-blue-600 rounded-full hover:bg-blue-50" data-note-id="${note.id}" title="编辑">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                </svg>
              </button>
              <button class="delete-note-btn p-2 text-gray-500 hover:text-red-600 rounded-full hover:bg-red-50" data-note-id="${note.id}" title="删除">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
